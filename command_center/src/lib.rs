use crate::kinode::process::llm::{
    claude_chat, embedding, groq_chat, openai_chat, register_claude_api_key,
    register_groq_api_key, register_openai_api_key,
};
...
fn llm() -> anyhow::Result<()> {
    ...
    // Register API keys
    println!(
        "OpenAI API key registered: {:?}",
        register_openai_api_key("sk-proj-J2y0MMBBYhLaw6iI680bT3BlbkFJPeH4fI3cumGNe6M6mbLX")
    );
    println!(
        "Groq API key registered: {:?}",
        register_groq_api_key("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")
    );
    println!("Claude API key registered: {:?}", register_claude_api_key("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"));

    // Get embedding
    let embedding = embedding("this is an embedding test", None);
    println!("Embedding result len: {:?}", embedding);

    // Chat with different models
    println!(
        "OpenAI chat result (default model): {:?}",
        openai_chat("What is the capital of France?", None)
    );
    println!(
        "OpenAI chat result (GPT-4): {:?}",
        openai_chat("What is the capital of France?", Some("gpt-4"))
    );
    println!(
        "Groq chat result (default model): {:?}",
        groq_chat("What is the capital of Germany?", None)
    );
    println!(
        "Claude chat result (default model): {:?}",
        claude_chat("What is the capital of Japan?", None)
    );
